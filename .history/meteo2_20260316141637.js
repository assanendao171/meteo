...
soleil.textContent=` ☀️ : région géographique ${input.value} , ${input.value.toUpperCase}`
degre.textContent=` 🌡️ Température ${reponse.name} : ${reponse.main.temp}°C`
neige.textContent=` ☁️ : Neige ${reponse.clouds.all}`
soleil.textContent=` 🌦️ : Bruine  ${input.value} , ${input.value.toUpperCase}`
soleil.textContent=` ☀️ ${input.value} , ${input.value.toUpperCase}`
      input.value=""
    }
    temps();
  } catch (Error) {
